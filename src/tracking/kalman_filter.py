...
class KalmanFilter:
    """Kalman filter for tracking bounding boxes in image space (x, y, aspect_ratio, height)."""
    ...
    def update(self, mean: np.ndarray, covariance: np.ndarray, measurement: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        chol_factor, lower = scipy_cholesky_safe(projected_cov)
...
def scipy_cholesky_safe(matrix: np.ndarray):
    try:
        from scipy.linalg import cho_factor
        return cho_factor(matrix, lower=True, check_finite=False)
    except Exception:
